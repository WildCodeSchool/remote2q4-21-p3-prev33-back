  ...
  connection.query(
    ...
    (err, results) => {
      if (err) {
        res
          .status(500)
          .send("Error retrieving training category from database");
      } else {
...
router.put("/:id", (req, res) => {
  ...
  db.query("SELECT * FROM training_category WHERE id = ?", [trainingCategoryId])
    .then(([results]) => {
      ...
      return db.query("UPDATE training_category SET ? WHERE id = ?", [
        req.body,
        trainingCategoryId,
      ]);
    })
    ...
    .catch((err) => {
      ...
      if (err === "RECORD_NOT_FOUND")
        res
          .status(404)
          .send(`trainingCategory with id ${trainingCategoryId} not found.`);
